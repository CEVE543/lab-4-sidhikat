...
I learned that some prior belifs could be very unrealistic, such as the first version, since it had very weak priors. Knowing that Houston area has a lot of rainfall, and is close to the coast, I increased the priors several times. I realized I had to increase the rainfall amounts more than I expected in order to get a curve that was similar to the Galveston GEV curve I got from Lab 3. I also reduced the uncertainty because I wanted it to remain within 2 $\theta\$. 
...
It seems like the observed data matched with my priors given the figure, but there is much more uncertainty in my prior values. The posterior distribution is much narrower. It seems Bayesian analysis values priors (domain knowledge) less if there is strong empirical evidence. 
...
The same priors should apply given that they are close geographically and should not have massive differences in rainfall amounts or climates. Although, it does seem like Alvin (my comparsion station) has a higher rainfall extreme and hence has a higher tail. But there is significant overlap in the return level uncertainties indicating that they are very similair. The difference between them could be due to difference in years of data, topography or proximity to a body of water. This implies that we should be mindful of spatial variability in creating regional assesments and change prior levels accordingly if needed.
