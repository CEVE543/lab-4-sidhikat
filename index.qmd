...
I learned that some prior belief could be very unrealistic, such as the first version, since it had very weak priors. Knowing that Houston area has a lot of rainfall, and is close to the coast, I increased the priors several times. I realized I had to increase the rainfall amounts more than I expected in order to get a curve that was similar to the Galveston GEV curve I got from Lab 3. I also reduced the uncertainty because I wanted it to remain within 2 $\sigma$.

**Response 2: Data Learning and Posterior Insights**

It seems like the observed data matched with my priors given the figure, but there is much more uncertainty in my prior values. In contrast, the posterior distribution using Bayesian analysis is much narrower. It seems that Bayesian analysis values priors (domain knowledge) less if there is strong, and a more robust set of empirical evidence. 

**Response 3: Two-Station Bayesian Comparison**

The same priors should apply given that the stations are close geographically and should not have significant differences in rainfall amounts or climates. But, Alvin (my comparsion station) has a higher rainfall extreme and hence has a higher tail. There is significant overlap in the return level uncertainties indicating that they are very similar. The difference between them could be due to difference in years of data, topography or proximity to a body of water. This implies that we should be mindful of spatial variability in creating regional assesments and change prior levels accordingly if needed.

**Response 4: Uncertainty Quantification and Model Validation**

Bayesian uncertainty accounts for priors and observational data while MLE only takes into account observational data, which underestimates extremes or overestimates noise. Diagnostic results such as $\hat{R}$ being 1,and mixed trace plots show that conergence was reached and the model is reliable.

**Response 5: Bayesian Workflow for PS1**

This iterative workflow experience will help in PS 1 by allowing me to identify accurate priors, and combine them with observed data for a more complete interpertation. Challenges I anticipate include selecting priors for regions with less data, and accurately interpreting spaitial variability.
...
#| output: false
return_level_priors = [
    ReturnLevelPrior(2, 5.0, 2.0),
    ReturnLevelPrior(10, 10, 2.5),
    ReturnLevelPrior(50, 15, 4),
    ReturnLevelPrior(100, 22, 5.5),
]
```
